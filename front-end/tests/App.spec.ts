...
test('load a non existent file', async ({ page }) => {
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill('load_file NotAFilePath without_header');
  await page.getByRole("button").click();
...
test('load a file with one column', async ({ page }) => {
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill('load_file FilePathOneColumn without_header');
  await page.getByRole("button").click();
...
test('load a file that is empty and viewing and searching it', async ({ page }) => {
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill('load_file FilePathEmpty without_header');
  await page.getByRole("button").click();
...
test('search a file with and without header', async ({ page }) => {
  ...
  await page.getByLabel("Command input").click();
  await page.getByLabel("Command input").fill('load_file RealFilePath1WithoutHeader without_header');
  await page.getByRole("button").click();
